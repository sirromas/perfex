...
class Projects_model extends CRM_Model
{
    ...
    public function get($id = '', $where = array())
    {
        ...
        if (is_numeric($id)) {
            ...
            $project = $this->db->get('tblprojects')->row();
            if ($project) {
                $project->shared_vault_entries = $this->clients_model->get_vault_entries($project->clientid, array(
                    'share_in_projects' => 1
                ));
    ...
    public function total_logged_time($id)
    {
        $q = $this->db->query('
            ...
            FROM tbltaskstimers
            WHERE task_id IN (SELECT id FROM tblstafftasks WHERE rel_type="project" AND rel_id=' . $id . ')')->row();
    ...
    public function new_project_file_notification($file_id, $project_id)
    {
        ...
        $this->send_project_email_template($project_id, 'new-project-file-uploaded-to-staff', 'new-project-file-uploaded-to-customer', $file->visible_to_customer, array(
            'staff' => array(
                'discussion_id' => $file_id,
