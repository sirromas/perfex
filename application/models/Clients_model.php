...
class Clients_model extends CRM_Model
{
     ...
     *            (optional) get all active or inactive
     * @return mixed Get client object based on passed clientid if not passed clientid return array of all clients
     */
     ...
     * @return int|string
     */
    public function get_color($clientid)
    {
     ...
     * @param mixed $id
     *            contact id
     * @param boolean $client_request
     *            is request from customers area
     * @return mixed
     */
     ...
     * @param mixed $customer_id
     *            customer id
     * @param boolean $not_manual_request
     *            is manual from admin area customer profile or register,convert to lead
     */
     ...
     *            data
     * @param
     *            client_request is this request from the customer area
     ...
     * @param
     *            integer ID
     * @return boolean Update client informations
     */
     ...
     * @param
     *            integer ID
     * @return boolean Delete client, also deleting rows from, dismissed client announcements, ticket replies, tickets, autologin, user notes
     */
     ...
     * @param
     *            integer Status ID
     * @return boolean Update contact status Active/Inactive
     */
     ...
     * @param
     *            integer Status ID
     * @return boolean Update client status Active/Inactive
     */
     ...
     * @param mixed $_POST
     *            data
     * @return mixed Change contact password, used from client area
     */
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        AND ' . $sqlDate . ' AND status != 5 and status != 6')->row()->invoiced_amount;
        ...
        FROM tblinvoicepaymentrecords
        JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid
        WHERE ' . $sqlDatePayments . ' AND tblinvoices.clientid = ' . $customer_id)->row()->amount_paid;
        ...
        $result['beginning_balance'] = $this->db->query('
            ...
            AND status != 6
            AND status != 5')->row()->beginning_balance;
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            foreach ($send_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $merge_fields = array();
                    $merge_fields = array_merge($merge_fields, get_client_contact_merge_fields($statement['client']->userid, $contact_id));
