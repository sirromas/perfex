...
class Tasks_model extends CRM_Model
{
        ...
        if ($insert_id) {
            ...
            if ($clientRequest == false) {
                ...
                if ($new_task_auto_assign_creator == true) {
                    $this->add_task_assignees(array(
                        'taskid' => $insert_id,
                        'assignee' => get_staff_user_id()
                    ));
                }
            }
    ...
    public function add_task_comment($data)
    {
        ...
        if ($insert_id) {
            ...
            do_action('task_comment_added', array(
                'task_id' => $task->id,
    ...
    public function mark_complete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            do_action('task_status_changed', array(
                'status' => 5,
                'task_id' => $id
            ));
     ...
     *            excluded staff id to not send the notifications
     * @return boolean
     */
    ...
    public function timer_tracking($task_id = '', $timer_id = '', $note = '')
    {
        ...
        if (total_rows('tbltaskstimers', array(
            ...
            do_action('task_timer_started', array(
                'task_id' => $task_id,
