...
function add_views_tracking($rel_type, $rel_id)
{
    ...
    if ($rel_type == 'invoice' || $rel_type == 'proposal' || $rel_type == 'estimate') {
        ...
        $CI->db->select('addedfrom,' . $responsible_column)->where('id', $rel_id);
...
function get_staff_full_name($userid = '')
{
    ...
    $staff = $CI->db->select('firstname,lastname')
        ->from('tblstaff')
        ->get()
        ->row();
    if ($staff) {
...
function get_contact_full_name($userid = '')
{
    ...
    $client = $CI->db->select('firstname,lastname')
        ->from('tblcontacts')
        ->get()
        ->row();
 ...
 *            main column in table for bettter performing
 * @param string $sTable
 *            table name
 * @param array $join
 *            join other tables
    ...
    if ($tags == '') {
        ...
        foreach ($tags_array as $tag) {
            ...
            if ($tag_row) {
                ...
                $CI->db->insert('tbltags', array(
                    'name' => $tag
                ));
                $tag_id = $CI->db->insert_id();
            }
            
            if (total_rows('tbltags_in', array(
                'tag_id' => $tag_id,
                'rel_id' => $rel_id,
                'rel_type' => $rel_type
            )) == 0) {
                $CI->db->insert('tbltags_in', array(
                    'tag_id' => $tag_id,
                    'rel_id' => $rel_id,
                    'rel_type' => $rel_type,
                    'tag_order' => $order
