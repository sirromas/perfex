 ...
 *            option from database to compare
 * @param string $label
 *            input label
...
function init_relation_tasks_table($table_attributes = array())
{
    ...
    if ($table_attributes['data-new-rel-type'] == 'project') {
        ...
        echo $CI->load->view('admin/tasks/_bulk_actions', array(
            'table' => '.table-rel-tasks'
        ), true);
        echo $CI->load->view('admin/tasks/_summary', array(
            'rel_id' => $table_attributes['data-new-rel-id'],
            'rel_type' => 'project',
            'table' => $table_name
        ), true);
            ...
            if ($type == 'lead') {
                ...
                if ($type == 'customer') {
                    if (! $has_permission_customers_view && ! have_assigned_customers() && $rel_id != $relation_values['id']) {
                        ...
                        if (have_assigned_customers() && $rel_id != $relation_values['id'] && ! $has_permission_customers_view) {
...
/**
 *
 * @deprecated Project status translate
 * @param mixed $id            
 * @return string
 */
function project_status_by_id($id)
{
    ...
    $hook_data = do_action('project_status_label', array(
        'id' => $id,
        'label' => $label
    ));
 ...
 *            default value
 * @param string $type
 *            input type eq text,number
 * @param array $input_attrs
 *            attributes on <input
 * @param array $form_group_attr
 *            <div class="form-group"> html attributes
 * @param string $form_group_class
 *            additional form group class
 * @param string $input_class
 *            additional class on input
 * @return string
 */
 ...
 *            input attributes
 * @param array $form_group_attr
 *            <div class="form-group"> div wrapper html attributes
 * @param string $form_group_class
 *            form group div wrapper additional class
 * @param string $input_class
 ...
 *            additional options attributes to include, attributes accepted based on the bootstrap-selectp lugin
 * @param string $label
 *            select label
 ...
 *            <div class="form-group"> div wrapper html attributes
 * @param string $form_group_class
 *            <div class="form-group"> additional class
 * @param string $select_class
 *            additional <select> class
 * @param boolean $include_blank
 *            do you want to include the first <option> to be empty
 * @return string
 */
...
<script
	src="<?php echo base_url('assets/plugins/jquery-validation/jquery.validate.min.js'); ?>"></script>
<script
	src="<?php echo base_url('assets/plugins/app-build/moment.min.js'); ?>"></script>
...
<?php
    ...
    if ($locale_key != 'en') {
        if (file_exists(FCPATH . 'assets/plugins/jquery-validation/localization/messages_' . $locale_key . '.min.js')) {
            ?>
<script
	src="<?php echo base_url('assets/plugins/jquery-validation/localization/messages_'.$locale_key.'.min.js'); ?>"></script>
...
<script
	src="<?php echo base_url('assets/plugins/jquery-validation/localization/messages_'.$locale_key.'_'.strtoupper($locale_key).'.min.js'); ?>"></script>
<?php } } ?>
<script
	src="<?php echo base_url('assets/plugins/datetimepicker/jquery.datetimepicker.full.min.js'); ?>"></script>
<script
	src="<?php echo base_url('assets/plugins/bootstrap-colorpicker/js/bootstrap-colorpicker.min.js'); ?>"></script>
<script>
    $(function(){
...
<link
	...
	rel="shortcut icon">
<?php } ?>
<link href='<?php echo base_url('assets/plugins/roboto/roboto.css'); ?>'
	rel='stylesheet'>
<link
...
<?php } ?>
<link
	href="<?php echo base_url('assets/plugins/datetimepicker/jquery.datetimepicker.min.css'); ?>"
	rel="stylesheet">
<link
...
<link
	...
	rel="stylesheet">
<link
	href="<?php echo base_url('assets/plugins/bootstrap-select/css/bootstrap-select.min.css'); ?>"
	rel="stylesheet">
 ...
 * 
 * @param boolean $aside
 *            should include aside
 */
function init_head($aside = true)
{
    $CI = & get_instance();
    $CI->load->view('admin/includes/head');
    $CI->load->view('admin/includes/header', array(
        'startedTimers' => $CI->misc_model->get_staff_started_timers()
    ));
 ...
 *            [description]
 * @param string $class
 *            table class / added prefix table-$class
 ...
 * @param array $headings            
 * @param string $class
 *            table class / add prefix eq.table-$class
 * @param array $additional_classes
 *            additional table classes
 * @param array $table_attributes
 *            table attributes
 * @param boolean $tfoot
 *            includes blank tfoot
 * @return string
 */
...
do_action('payment_gateway_scripts');
    $contents = ob_get_contents();
    ob_end_clean();
    return $contents;
}
 ...
 *            include checkboxes on the left side for bulk actions
 */
function AdminTicketsTableStructure($name = '', $bulk_action = false)
{
    ...
    ?>
<table
	class="table dt-table-loading <?php
    echo $name;
    ?> table-tickets">
	<thead>
		<tr>
...
<th><span class="hide"> - </span>
			<div class="checkbox mass_select_all_wrap">
					<input type="checkbox" id="mass_select_all" data-to-table="tickets"><label></label>
				</div></th>
<?php
    }
    ?>
<th>#</th>
    ...
    ?></th>
			<th
				<?php if(get_option('services') == 0){echo ' class="not_visible"'; }?>><?php
...
function format_task_status($status, $text = false, $clean = false)
{
    ...
    $hook_data = do_action('task_status_name', array(
        'current' => $status_name,
        'status_id' => $status['id']
    ));
...
if (! function_exists('get_table_items_and_taxes')) {
     ...
     *            all items
     * @param [type] $type
     *            where do items come form, eq invoice,estimate,proposal etc..
     * @param boolean $admin_preview
     *            in admin preview add additional sortable classes
     * @return array
     */
 ...
 *            prevent specific tags to be stripped
 * @return string
 */
