...
<a href="#" class="btn btn-default" onclick="taskTable(); return false;"><i
...
foreach ($milestones as $milestone) {
            $tasks = $this->projects_model->get_tasks($project->id, array(
            ...
            ?>
      <div
			class="col-md-4 mtop25<?php if($milestone['id'] == 0 && count($tasks) == 0){echo ' hide'; } ?>">
			<div
				class="panel-heading <?php if($milestone_color != ''){echo 'color-not-auto-adjusted color-white ';} ?><?php if($milestone['id'] != 0){echo 'task-phase';}else{echo 'info-bg';} ?>"
...
echo '<br /><small>' . _l('milestone_total_logged_time') . ': ' . seconds_to_time_format($milestone['total_logged_time']) . '</small>';
            ...
            foreach ($tasks as $task) {
                ?>
            <div
					class="media _task_wrapper<?php if((!empty($task['duedate']) && $task['duedate'] < date('Y-m-d')) && $task['status'] != 5){ echo ' overdue-task'; } ?>">
					<div class="media-body">
						<a
							href="<?php echo site_url('clients/project/'.$project->id.'?group=project_tasks&taskid='.$task['id']); ?>"
							class="task_milestone pull-left<?php if($task['status'] == 5){echo ' line-throught text-muted';} ?>"><?php echo $task['name']; ?></a>
                  <?php if($project->settings->edit_tasks == 1 && $task['is_added_from_contact'] == 1 && $task['addedfrom'] == get_contact_user_id()){ ?>
                  <a
							href="<?php echo site_url('clients/project/'.$project->id.'?group=edit_task&taskid='.$task['id']); ?>"
							class="pull-right"> <small><i class="fa fa-pencil-square-o"></i></small>
						</a>
                  <?php } ?>
                  <br /> <small><?php echo _l('task_status'); ?>: <?php echo format_task_status($task['status'],true); ?></small>
			...
			<div class="panel-footer">
				<div class="progress no-margin progress-bg-dark">
					<div class="progress-bar progress-bar-success" role="progressbar"
						...
						style="width: 0%" data-percent="<?php echo $percent; ?>"></div>
				</div>
			</div>
		</div>
      <?php } ?>
...
<div
	class="tasks-table <?php if($project->settings->view_milestones == 1){echo 'hide';} ?>">
   ...
   <div class="btn-group mtop15">
		...
		<ul class="dropdown-menu width300">
			<li><a href="#"
				onclick="dt_custom_view('.table-tasks',4,''); return false;">
            <?php echo _l('task_list_all'); ?>
            </a></li>
         <?php foreach($tasks_statuses as $status){ ?>
         <li><a href="#"
				onclick="dt_custom_view('.table-tasks',4,'<?php echo $status['name']; ?>'); return false;">
	...
	<div class="table-responsive">
		<table class="table dt-table table-tasks" data-order-col="0"
			data-order-type="asc">
			<thead>
				<tr>
					<!-- milestone order -->
        ...
        foreach ($project_tasks as $task) {
            ?>
            <tr
					class="<?php if($task['status'] != 5){echo 'task-unfinished-table';} ?>">
					<td class="hide"
						data-order="<?php echo $task['milestone_order']; ?>"></td>
					<td>
                  <?php if($project->settings->edit_tasks == 1 && $task['is_added_from_contact'] == 1 && $task['addedfrom'] == get_contact_user_id()){ ?>
                  <a
