<div class="row">
    ...
    <h2 class="no-mtop" id="task-edit-heading"><?php echo $task->name; ?></h2>
		...
		<div class="form-group">
			...
			<input type="text" name="name" id="name" class="form-control"
				required value="<?php echo $task->name; ?>">
		</div>
		<div class="row">
			<div
				class="col-md-<?php if($project->settings->view_milestones == 1){ echo 6; } else {echo 12;} ?>">
				<div class="form-group">
					...
					<select name="priority" class="selectpicker" id="priority"
						...
						<option value="1"
							<?php if($task->priority == 1){echo 'selected';} ?>><?php echo _l('task_priority_low'); ?></option>
						<option value="2"
							<?php if($task->priority == 2){echo 'selected';} ?>><?php echo _l('task_priority_medium'); ?></option>
						<option value="3"
							<?php if($task->priority == 3){echo 'selected';} ?>><?php echo _l('task_priority_high'); ?></option>
						<option value="4"
							<?php if($task->priority == 4){echo 'selected';} ?>><?php echo _l('task_priority_urgent'); ?></option>
      ...
      <div class="col-md-6">
            ...
            <option value="<?php echo $milestone['id']; ?>"
							<?php if($milestone['id'] == $task->milestone){echo ' selected';} ?>><?php echo $milestone['name']; ?></option>
            <?php } ?>
          </select>
				</div>
			</div>
   ...
   <div class="form-group">
			...
			<select class="selectpicker" multiple="true" name="assignees[]"
				id="assignees" data-width="100%" data-live-search="true">
      <?php foreach($members as $member){ ?>
      <option value="<?php echo $member['staff_id']; ?>"
					<?php if($this->tasks_model->is_task_assignee($member['staff_id'],$task->id)){echo ' selected';} ?>><?php echo get_staff_full_name($member['staff_id']); ?></option>
      <?php } ?>
    </select>
		</div>
  <?php } ?>
