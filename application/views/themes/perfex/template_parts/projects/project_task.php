		...
		<div class="col-md-12">
			<div class="row">
               ...
               <?php } ?>
				...
				<div class="col-md-5">
					...
					<div
						class="task-info pull-right <?php if(!$view_task->status != 5){echo ' text-danger'; }else{echo 'text-info';} ?><?php if(!$view_task->duedate){ echo ' hide';} ?>">
						<h5 class="no-margin">
               ...
               <?php if($view_task->status == 2){ ?>
               <div class="pull-right task-info text-success">
						<h5 class="no-margin">
							<i class="fa fa-check"></i>
                     <?php echo _l('task_single_finished'); ?>: <?php echo _d($view_task->datefinished); ?>
                  </h5>
					</div>
               <?php } ?>
               <div class="clearfix"></div>
         ...
         <div class="clearfix"></div>
            ...
            foreach ($view_task->assignees as $assignee) {
                $_assignees .= '
                   <div data-toggle="tooltip" class="pull-left mleft5 task-user" data-title="' . get_staff_full_name($assignee['assigneeid']) . '">' . staff_profile_image($assignee['assigneeid'], array(
         ...
         <p
				class="<?php if($list['finished'] == 1){echo 'line-throught';} ?>">
				<span
					class="task-checklist-indicator <?php if($list['finished'] == 1){echo 'text-success';} else{echo 'text-muted';} ?>"><i
...
$value = get_custom_field_value($view_task->id, $field['id'], 'tasks');
                if ($value == '') {
                    continue;
                }
                ...
                ?>
            <div class="col-md-9">
					<span class="bold"><?php echo ucfirst($field['name']); ?></span> <br />
					<div class="text-left">
                  <?php echo $value; ?>
               </div>
				</div>
				...
				<div class="col-md-12">
                  ...
                  <div class="col-md-4">
                    ...
                    ?>
                              ...
                              <a
											href="<?php echo site_url('clients/delete_file/'.$attachment['id'].'/task?project_id='.$project->id); ?>"
											class="text-danger _delete pull-right"><i
                    ...
                    if (empty($attachment['external'])) {
                        ...
                        if (! empty($attachment['thumbnail_link']) || ! empty($attachment['external'])) {
                    ...
                    ?>
                           <div
										class="<?php echo ($is_image ? 'preview_image' : 'task-attachment-no-preview'); ?>">
										<a href="<?php echo $href_url; ?>" <?php if($is_image){ ?>
         ...
         <input type="file" name="file" multiple class="hide" /> <input
				type="hidden" name="task_id" value="<?php echo $view_task->id; ?>"
				class="hide" />
         <?php echo form_close(); ?>
...
echo staff_profile_image($comment['staffid'], array(
            ...
            <?php } else { ?>
            <img
					src="<?php echo contact_profile_image_url($comment['contact_id']); ?>"
            ...
            <div class="media-body">
               ...
               <?php
                        ...
                        if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour)) {
                            ?>
               <a href="#"
						onclick="remove_task_comment(<?php echo $comment['id']; ?>); return false;"
						class="pull-right"> <i class="fa fa-times text-danger"></i>
					</a> <a href="#"
						onclick="edit_task_comment(<?php echo $comment['id']; ?>); return false;"
						class="pull-right mright5"> <i class="fa fa-pencil-square-o"></i>
					</a>
               ...
               <div class="comment-content"
						data-comment-content="<?php echo $comment['id'] ;?>">
                  <?php
