	...
	<table class="table dt-table" data-order-col="3" data-order-type="desc">
		...
		<tbody>
            ...
            <tr>
				...
				<td><a
					href="<?php echo site_url('clients/project/'.$project->id.'?group=project_tasks&taskid='.$timesheet['task_data']->id); ?>"><?php echo $timesheet['task_data']->name; ?></a></td>
				<td
					data-order="<?php echo strftime('%Y-%m-%d %H:%M:%S', $timesheet['start_time']); ?>">
                    <?php echo _dt($timesheet['start_time'],true); ?>
                </td>
				<td
					data-order="<?php if(!is_null($timesheet['end_time'])){echo strftime('%Y-%m-%d %H:%M:%S', $timesheet['end_time']);} ?>"><?php
