							...
							<div class="col-md-8">
								<div class="ticket-status-inline">
									<span class="label pull-right bold" style="background:<?php echo $ticket->statuscolor; ?>">
                                        ...
                                        <i
										class="fa fa-pencil-square-o pointer toggle-change-ticket-status"></i>
									</span>
                                        <?php } ?>
                                    </div>
                                    <?php if(get_option('allow_customer_to_change_ticket_status') == 1){ ?>
...
foreach ($ticket_statuses as $status) {
                                            if ($status['ticketstatusid'] == 3 || $status['ticketstatusid'] == 4) {
                                                continue;
                                            }
                                            ?>
                                                    <option
												value="<?php echo $status['ticketstatusid']; ?>"
												<?php if($status['ticketstatusid'] == $ticket->ticketstatusid){echo 'selected';}?>>
                                                        <?php echo ticket_status_translate($status['ticketstatusid']); ?>
                                                    </option>
                                                    <?php } ?>
                                                </select> <span
											class="input-group-addon"><i
											class="fa fa-remove pointer toggle-change-ticket-status"></i></span>
									</div>
								</div>
                                        <?php } ?>
                                    </div>
						...
						<p>
                                ...
                                foreach ($custom_fields as $field) {
                                    ?>
                                <hr />
						<p class="bold"><?php echo $field['name']; ?>: <span
								class="pull-right bold"><?php echo get_custom_field_value($ticket->ticketid,$field['id'],'tickets'); ?></span>
						</p>
                                <?php } ?>
                            </div>
				</div>
	...
	<div class="col-md-8">
		...
		<div class="panel_s single-ticket-reply-area"
			<?php if (form_error('message') == ''){ ?> style="display: none;"
			<?php } ?>>
			<div class="panel-heading">
			...
			<div class="row">
				<div class="col-md-12 mtop20 text-center">
					<button class="btn btn-info" type="submit"
						data-form="#ticket-reply" autocomplete="off"
						data-loading-text="<?php echo _l('wait_text'); ?>"><?php echo _l('ticket_single_add_reply'); ?></button>
				</div>
			</div>
		</div>
        <?php echo form_close(); ?>
