...
<aside id="menu" class="sidebar">
	<ul class="nav metis-menu" id="side-menu">
		<li
			class="dashboard_user<?php if($total_qa_removed == count($quickActions)){echo ' dashboard-user-no-qa';}?>">
   <?php echo _l('welcome_top',$current_user->firstname); ?> <i
			class="fa fa-power-off top-left-logout pull-right"
			data-toggle="tooltip" data-title="<?php echo _l('nav_logout'); ?>"
 ...
 <li class="quick-links">
			<div class="dropdown dropdown-quick-links">
				<a href="#" class="dropdown-toggle" id="dropdownQuickLinks"
					...
					class="fa fa-gavel" aria-hidden="true"></i>
				</a>
    ...
    foreach ($quickActions as $key => $item) {
        ...
        ?>
      <li><a href="<?php echo $url; ?>" <?php echo $href_attributes; ?>>
...
foreach ($menu_active->aside_menu_active as $item) {
    if ($item->id == 'tickets' && (get_option('access_tickets_to_none_staff_members') == 0 && ! is_staff_member())) {
        ...
        )) > 0 && get_option('allow_staff_view_proposals_assigned') == 1) && (! has_permission('proposals', '', 'view') && ! has_permission('proposals', '', 'view_own'))) {
            $item->permission = '';
        }
    }
    if (! empty($item->permission) && ! has_permission($item->permission, '', 'view') && ! has_permission($item->permission, '', 'view_own')) {
        continue;
    }
    $submenu = false;
    ...
    if (isset($item->children)) {
        ...
        foreach ($item->children as $_sub_menu_check) {
            if (! empty($_sub_menu_check->permission) && ($_sub_menu_check->permission != 'payments' && $_sub_menu_check->permission != 'tickets' && $_sub_menu_check->permission != 'customers' && $_sub_menu_check->permission != 'proposals')) {
                ...
                if ((get_option('allow_staff_view_proposals_assigned') == 0 || (get_option('allow_staff_view_proposals_assigned') == 1 && total_rows('tblproposals', array(
                    'assigned' => get_staff_user_id()
                )) == 0)) && ! has_permission('proposals', '', 'view') && ! has_permission('proposals', '', 'view_own')) {
                    $total_sub_items_removed ++;
                }
            }
        }
    ...
    ?>
<li class="menu-item-<?php echo $item->id; ?>"><a
			href="<?php echo $url; ?>" aria-expanded="false"><i
...
foreach ($item->children as $submenu) {
            if (! empty($submenu->permission) && ($submenu->permission != 'payments' && $submenu->permission != 'tickets' && $submenu->permission != 'proposals' && $submenu->permission != 'customers') && (! has_permission($submenu->permission, '', 'view') && ! has_permission($submenu->permission, '', 'view_own'))) {
                continue;
            } elseif ($submenu->permission == 'payments' && (! has_permission('payments', '', 'view') && ! has_permission('invoices', '', 'view_own'))) {
                continue;
                ...
                if ((total_rows('tblproposals', array(
                    'assigned' => get_staff_user_id()
                )) > 0 && get_option('allow_staff_view_proposals_assigned') == 0) && (! has_permission('proposals', '', 'view') && ! has_permission('proposals', '', 'view_own'))) {
                    continue;
                }
            }
            $url = $submenu->url;
            ...
            ?>
<li class="sub-menu-item-<?php echo $submenu->id; ?>"><a
					href="<?php echo $url; ?>">
 <?php if(!empty($submenu->icon)){ ?>
 <i class="<?php echo $submenu->icon; ?> menu-icon"></i>
 <?php } ?>
 <?php echo _l($submenu->name); ?></a></li>
<?php } ?>
</ul>
<?php } ?>
</li>
...
<li
			<?php if(get_option('show_setup_menu_item_only_on_hover') == 1) { echo ' style="display:none;"'; } ?>
			id="setup-menu-item"><a href="#" class="open-customizer"><i
				class="fa fa-cog menu-icon"></i>
...
<li class="pinned_project"><a
			href="<?php echo admin_url('projects/view/'.$pinnedProject['id']); ?>"
			data-toggle="tooltip"
