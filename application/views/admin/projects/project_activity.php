...
foreach ($activity as $activity) {
        ...
        if ($activity['staff_id'] != 0) {
            ?>
                      <a
						href="<?php echo admin_url('profile/'.$activity['staff_id']); ?>"><?php echo staff_profile_image($activity['staff_id'],array('staff-profile-xs-image','pull-left mright10')); ?></a>
                      <?php
            ...
            if ($activity['contact_id'] != 0) {
                ...
                ?>
						...
						<img
						src="<?php echo contact_profile_image_url($activity['contact_id']); ?>"
						class="staff-profile-xs-image pull-left mright10">
					</a>
                        <?php } ?>
                        <?php
        
if ($activity['visible_to_customer'] == 1) {
            $checked = 'checked';
        } else {
            $checked = '';
        }
        ?>
					...
					<p class="no-mbot text-muted mleft30"><?php echo $activity['additional_data']; ?></p>
				</div>
			</div>
			<div class="col-md-4 text-right">
				...
				<div class="pull-right">
					<div class="onoffswitch">
						<input type="checkbox"
							<?php if(!has_permission('projects','','create')){echo 'disabled';} ?>
							...
							<?php echo $checked; ?>> <label class="onoffswitch-label"
							for="<?php echo $activity['id']; ?>"></label>
					</div>
				</div>
			</div>
