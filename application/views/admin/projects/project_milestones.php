<?php
if(total_rows('tblstafftasks',array('rel_id'=>$project->id,'rel_type'=>'project')) == 0){
...
<div class="row">
  ...
  foreach($milestones as $milestone){
    $milestonesTasksWhere = array('milestone'=>$milestone['id']);
  ...
  ?>
  <div class="col-md-3 mtop25 milestone-column<?php if($milestone['id'] == 0 && count($tasks) == 0){echo ' hide';} ?>" data-milestone-id="<?php echo $milestone['id']; ?>">
...
<div class="panel-body">
  ...
  foreach($tasks as $task){
   ...
   ?>
   <li data-task-id="<?php echo $task['id']; ?>" class="task<?php if(has_permission('tasks','','create') || has_permission('tasks','','edit')){echo ' sortable';} ?><?php if($current_user_is_assigned){echo ' current-user-task';} if((!empty($task['duedate']) && $task['duedate'] < date('Y-m-d')) && $task['status'] != 5){ echo ' overdue-task'; } ?>">
    <div class="media">
      <?php
      if(count($assignees) > 0){ ?>
      <div class="media-left">
        <?php if($current_user_is_assigned){
       ...
       foreach($assignees as $assigned){
        if($assigned['assigneeid'] != get_staff_user_id()){
         echo staff_profile_image($assigned['assigneeid'],array('staff-profile-image-xs sub-staff-assigned-milestone pull-left'),'small',array('data-toggle'=>'tooltip','data-title'=>$assigned['firstname'] . ' ' . $assigned['lastname']));
       }
     }
     ?>
   </div>
