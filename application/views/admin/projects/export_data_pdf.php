...
$html .= '<h3>' . _l('project_overview') . '</h3>';
...
$html = '';
$html .= '<b style="background-color:#f0f0f0;">' . _l('finance_overview') . '</b><br /><br />';
...
$html .= '<b>' . _l('project_overview_expenses') . ': </b>' . format_money(sum_from_table('tblexpenses', array(
    'where' => array(
        'project_id' => $project->id
    ),
    'field' => 'amount'
)), $project->currency_data->symbol) . '<br />';
// Billable expenses + money
$html .= '<b>' . _l('project_overview_expenses_billable') . ': </b>' . format_money(sum_from_table('tblexpenses', array(
    'where' => array(
        'project_id' => $project->id,
        'billable' => 1
    ),
...
$html .= '<b>' . _l('project_overview_expenses_billed') . ': </b>' . format_money(sum_from_table('tblexpenses', array(
    'where' => array(
        'project_id' => $project->id,
        'invoiceid !=' => 'NULL',
        'billable' => 1
...
$html .= '<b>' . _l('project_overview_expenses_unbilled') . ': </b>' . format_money(sum_from_table('tblexpenses', array(
    'where' => array(
        'project_id' => $project->id,
        'invoiceid IS NULL',
        'billable' => 1
...
if (count($custom_fields) > 0) {
    $html = '';
    $html .= '<b style="background-color:#f0f0f0;">' . _l('project_custom_fields') . '</b><br /><br />';
...
$html = '';
// Customer Info
$html .= '<b style="background-color:#f0f0f0;">' . _l('project_customer') . '</b><br /><br /><b>' . $project->client_data->company . '</b><br />';
...
if (! empty($project->client_data->vat)) {
    $html .= '<br />' . _l('invoice_vat') . ': ' . $project->client_data->vat;
}
...
$html = '';
// Heading
$html .= '<p><b style="background-color:#f0f0f0;">' . _l('project_members_overview') . '</b></p>';
...
foreach ($members as $member) {
    ...
    $html .= '<td>' . total_rows('tblprojectdiscussions', array(
    ...
    $html .= '<td>' . total_rows('tblprojectfiles', array(
        'staffid' => $member['staff_id'],
        'project_id' => $project->id
    )) . '</td>';
    $member_tasks_assigned = $this->tasks_model->get_tasks_by_staff_id($member['staff_id'], array(
        'rel_id' => $project->id,
        'rel_type' => 'project'
    ));
    $seconds = 0;
...
$pdf->Ln(5);
$html = '';
$html .= '<p><b style="background-color:#f0f0f0;">' . _l('detailed_overview') . '</b></p>';
$html .= '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="1">';
$html .= '<thead>';
$html .= '<tr bgcolor="#323a45" style="color:#ffffff;">';
...
foreach ($tasks as $task) {
    $logged_time = $task['total_logged_time'];
    $html .= '<tr style="color:#4a4a4a;">';
    $html .= '<td width="26.12%">' . $task['name'] . '</td>';
    $html .= '<td width="12%">' . total_rows('tblstafftaskassignees', array(
        'taskid' => $task['id']
    )) . '</td>';
    $html .= '<td width="12%">' . total_rows('tblstafftasksfollowers', array(
        'taskid' => $task['id']
    )) . '</td>';
    ...
    $html .= '<td width="14.28%">' . seconds_to_time_format($logged_time) . '</td>';
    $html .= '<td width="10%">' . sec2qty($logged_time) . '</td>';
...
$pdf->Ln(5);
$html = '';
$html .= '<p><b style="background-color:#f0f0f0;">' . _l('timesheets_overview') . '</b></p>';
$html .= '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="1">';
$html .= '<thead>';
$html .= '<tr bgcolor="#323a45" style="color:#ffffff;">';
...
$pdf->Ln(5);
$html = '';
$html .= '<p><b style="background-color:#f0f0f0;">' . _l('project_milestones_overview') . '</b></p>';
$html .= '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="1">';
$html .= '<thead>';
$html .= '<tr bgcolor="#323a45" style="color:#ffffff;">';
...
foreach ($milestones as $milestone) {
    ...
    $html .= '<td width="15%">' . total_rows('tblstafftasks', array(
