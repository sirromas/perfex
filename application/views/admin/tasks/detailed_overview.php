	...
	<div class="content">
		<div class="row">
            ...
            <a href="<?php echo admin_url('tasks'); ?>"
							class="btn btn-default pull-left"><?php echo _l('back_to_tasks_list'); ?></a>
            <?php } else { ?>
            <a
							href="<?php echo admin_url('projects/view/'.$this->input->get('project_id').'?group=project_tasks'); ?>"
            ...
            <div class="row">
              ...
              <div class="col-md-2 border-right">
                <?php
                echo render_select('member', $members, array(
                    'staffid',
                    array(
                        'firstname',
                        'lastname'
                    ...
                    'data-none-selected-text' => _l('all_staff_members')
                ), array(), 'no-margin');
                ?>
              </div>
              <?php } ?>
              <div class="col-md-2 border-right">
                ...
                echo render_select('month', $months, array(
                    'month',
                    array(
                        'name'
                    )
                ), '', $selected, array(
                    'data-none-selected-text' => _l('task_filter_detailed_all_months')
                ), array(), 'no-margin');
                ?>
              </div>
                    ...
                    <option value="<?php echo $status['id']; ?>"
											<?php if($this->input->post('status') == $status['id']){echo 'selected'; } ?>><?php echo $status['name']; ?></option>
                    <?php } ?>
                  </select>
								</div>
							</div>
							<div class="col-md-2 border-right">
								<select name="year" id="year" class="selectpicker no-margin"
									data-width="100%">
                <?php foreach($years as $data){ ?>
                <option value="<?php echo $data['year']; ?>"
										<?php if($this->input->post('year') == $data['year'] || date('Y') == $data['year']){echo 'selected'; } ?>><?php echo $data['year']; ?></option>
                <?php } ?>
              </select>
							</div>
							<div class="col-md-2">
          ...
          <h4 class="no-margin bold text-success"><?php echo  _l(date('F', mktime(0, 0, 0, $month, 1))); ?>
            ...
            foreach ($data as $task) {
                ...
                <tr class="<?php echo $row_class; ?>">
										<td class="stripped-table-data" width="60%"><a href="#"
                ...
                ?>
                    <div class="pull-right">
												...
												<span class="label label-default-light pull-left mright5"
													data-toggle="tooltip"
													data-title="<?php echo mb_substr(_l('task_total_logged_time'),0,-1); ?>">
                ...
                if (total_rows('tbltaskchecklists', array(
                    ...
                    if ((total_rows('tbltaskchecklists', array(
                        'finished' => 1,
                        'taskid' => $task['id']
                    )) != total_rows('tbltaskchecklists', array(
                        'taskid' => $task['id']
                    )))) {
                        echo 'label-danger';
                    } else 
                ...
                ?> pull-left mright5"
                    ...
                    <?php
                $where_checklists_finished = array(
                    'finished' => 1,
                    'taskid' => $task['id']
                );
                if (is_numeric($staff_id)) {
                ...
                ?>
                    <i class="fa fa-th-list"></i> <?php echo total_rows('tbltaskchecklists',$where_checklists_finished); ?>/<?php echo total_rows('tbltaskchecklists',array('taskid'=>$task['id'])); ?>
													...
													class="label label-default-light pull-left mright5"
													data-toggle="tooltip"
													data-title="<?php echo _l('tasks_total_added_attachments'); ?>">
                   <?php
                $where_comments = array(
                    'taskid' => $task['id']
                );
                $where_files = array(
                    'rel_id' => $task['id'],
                ...
                ?>
                  <i class="fa fa-paperclip"></i> <?php echo total_rows('tblfiles',$where_files); ?>
													...
													class="label label-default-light pull-left"
													data-toggle="tooltip"
													data-title="<?php echo _l('tasks_total_comments'); ?>"> <i
													class="fa fa-comments"></i> <?php echo total_rows('tblstafftaskcomments',$where_comments); ?>
               </span>
											</div></td>
