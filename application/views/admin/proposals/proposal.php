                     ...
                     <div class="col-md-6 border-right">
                        ...
                        <div class="form-group">
									...
									<select name="rel_type" id="rel_type" class="selectpicker"
										...
										<option value=""></option>
										<option value="lead"
											<?php if((isset($proposal) && $proposal->rel_type == 'lead') || $this->input->get('rel_type')){if($rel_type == 'lead'){echo 'selected';}} ?>><?php echo _l('proposal_for_lead'); ?></option>
										<option value="customer"
											<?php if((isset($proposal) &&  $proposal->rel_type == 'customer') || $this->input->get('rel_type')){if($rel_type == 'customer'){echo 'selected';}} ?>><?php echo _l('proposal_for_customer'); ?></option>
									</select>
								</div>
								<div class="form-group<?php if($rel_id == ''){echo ' hide';} ?>"
									...
									<div id="rel_id_select">
										<select name="rel_id" id="rel_id" class="ajax-search"
...
if ($rel_id != '' && $rel_type != '') {
                        ...
                        ?>
                        <?php
                        echo render_select('currency', $currencies, array(
                            'id',
                            'name',
                            'symbol'
                        ), 'proposal_currency', $selected, do_action('proposal_currency_disabled', $s_attrs));
                        ?>
                        <div class="form-group">
        ...
        ?>><?php echo _l('discount_type_before_tax'); ?></option>
                        ...
                        <div class="form-group">
									<div class="checkbox checkbox-primary" data-toggle="tooltip"
										title="<?php echo _l('proposal_allow_comments_help'); ?>">
										<input type="checkbox" name="allow_comments"
											id="allow_comments"
											<?php if(isset($proposal)){if($proposal->allow_comments == 1){echo 'checked';}}; ?>>
                                ...
                                ?>
                                 <select name="status"
                                    ...
                                    <option
													...
													<?php if((isset($proposal) && $proposal->status == $status) || (!isset($proposal) && $status == 0)){echo 'selected';} ?>><?php echo format_proposal_status($status,'',false); ?></option>
                                    <?php } ?>
                                 </select>
										</div>
									</div>
                            ...
                            echo render_select('assigned', $staff, array(
                                'staffid',
                                array(
                                    'firstname',
                                    'lastname'
                                )
                            ), 'proposal_assigned', $selected);
                            ?>
                           </div>
								</div>
						...
						<div class="btn-bottom-toolbar text-right">
							...
							<button type="button"
								class="btn btn-info mleft10 proposal-form-submit save-and-send">
                        <?php echo _l('save_and_send'); ?>
                    </button>
							<button class="btn btn-info mleft5 proposal-form-submit"
