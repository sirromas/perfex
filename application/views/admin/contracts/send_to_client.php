<div class="modal fade email-template"
	data-editor-id=".<?php echo 'tinymce-'.$contract->id; ?>"
	id="contract_send_to_client_modal" tabindex="-1" role="dialog"
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
                            ...
                            echo render_select('sent_to[]', $contacts, array(
                                'id',
                                'email',
                                'firstname,lastname'
                            ), 'contract_send_to', $selected, array(
                                'multiple' => true
                            ), array(), '', '', false);
						...
						<div class="checkbox checkbox-primary">
							<input type="checkbox"
								<?php if(empty($contract->content)){echo 'disabled';} else {echo 'checked';} ?>
								name="attach_pdf" id="attach_pdf"> <label for="attach_pdf"><?php echo _l('contract_send_to_client_attach_pdf'); ?></label>
						</div>
					...
					<div class="col-md-12">
						...
						<hr />
                            <?php foreach($contract->attachments as $attachment) { ?>
                                <div class="checkbox checkbox-primary">
							<input type="checkbox"
								<?php if(!empty($attachment['external'])){echo 'disabled';}; ?>
								value="<?php echo $attachment['id']; ?>"
								name="email_attachments[]"> <label for=""><a
								href="<?php echo site_url('download/file/contract/'.$attachment['id']); ?>"><?php echo $attachment['file_name']; ?></a></label>
						</div>
                                <?php } ?>
                            </div>
				</div>
