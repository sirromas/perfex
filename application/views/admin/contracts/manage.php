		...
		<div class="row">
			<div class="col-md-12">
				<div class="panel_s">
                    ...
                    <a
							href="<?php echo admin_url('contracts/contract'); ?>"
							class="btn btn-info pull-left display-block"><?php echo _l('new_contract'); ?></a>
                    <?php } ?>
                    <div
							...
							<ul class="dropdown-menu dropdown-menu-left width300 height500">
								<li class="active"><a href="#"
									data-cview="exclude_trashed_contracts"
                            ...
                            <div class="clearfix"></div>
								...
								<li class="dropdown-submenu pull-left"><a href="#" tabindex="-1"><?php echo _l('months'); ?></a>
                                    ...
                                    <li><a href="#"
											...
											onclick="dt_custom_view(<?php echo $m; ?>,'.table-contracts','contracts_by_month_<?php echo $m; ?>'); return false;"><?php echo _l(date('F', mktime(0, 0, 0, $m, 1))); ?></a></li>
                                    <?php } ?>
                                </ul></li>
								<div class="clearfix"></div>
...
$plus_7_days = date('Y-m-d', strtotime("+7 days"));
                        ...
                        <div class="col-md-12">
							...
							<div class="col-md-2 col-xs-6 border-right">
								<h3 class="bold"><?php
        echo total_rows('tblcontracts', 'dateend BETWEEN "' . $minus_7_days . '" AND "' . $plus_7_days . '" AND trash=0 AND dateend is NOT NULL AND dateend >"' . date('Y-m-d') . '"' . (count($where_own) > 0 ? ' AND addedfrom=' . get_staff_user_id() : ''));
        ?></h3>
								<span class="text-warning"><?php echo _l('contract_summary_about_to_expire'); ?></span>
							</div>
							<div class="col-md-2 col-xs-6 border-right">
								<h3 class="bold"><?php
        echo total_rows('tblcontracts', 'dateadded BETWEEN "' . $minus_7_days . '" AND "' . $plus_7_days . '" AND trash=0' . (count($where_own) > 0 ? ' AND addedfrom=' . get_staff_user_id() : ''));
        ?></h3>
...
<script>
    $(function(){
        ...
        initDataTable('.table-contracts', window.location.href, [not_sortable_contracts], [not_sortable_contracts], ContractsServerParams,<?php echo do_action('contracts_table_default_order',json_encode(array(6,'ASC'))); ?>);
