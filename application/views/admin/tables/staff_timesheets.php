...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, $additionalSelect, ($this->_instance->input->post('group_by_task') ? 'GROUP BY task_id' : ''));
...
if ($chart_type == 'today') {
    $footer_data['chart']['data'] = array(
        sec2qty(array_sum($footer_data['chart']['data']))
    );
} elseif ($chart_type == 'week') {
