...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array(
    ...
    'userid',
    'is_primary',
    '(SELECT count(*) FROM tblcontacts c WHERE c.userid=tblcontacts.userid) as total_contacts'
));
...
foreach ($rResult as $aRow) {
    ...
    $row[] = (! empty($aRow['last_login']) ? time_ago($aRow['last_login']) : '');
    ...
    $options .= icon_btn('#', 'pencil-square-o', 'btn-default', array(
        'onclick' => 'contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;'
    ));
