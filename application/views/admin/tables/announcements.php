...
foreach ($rResult as $aRow) {
    ...
    if (total_rows('tbldismissedannouncements', array(
        'announcementid' => $aRow['announcementid'],
        'staff' => 1,
        'userid' => get_staff_user_id()
    )) == 0 && $aRow['showtostaff'] == 1) {
        $options .= icon_btn('misc/dismiss_announcement/' . $aRow['announcementid'], 'check', 'btn-success', array(
            'data-toggle' => 'tooltip',
            'data-title' => _l('dismiss_announcement')
        ));
    }
    if (is_admin()) {
