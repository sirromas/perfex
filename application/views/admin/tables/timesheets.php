...
$where = array(
    'AND task_id IN (SELECT id FROM tblstafftasks WHERE rel_id="' . $project_id . '" AND rel_type="project")'
);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i ++) {
        ...
        $user_removed_as_assignee = (total_rows('tblstafftaskassignees', array(
        ...
        if ($i == 0) {
            ...
            if ($aColumns[$i] == 'task_id') {
                ...
                if ($aRow['billed'] == 1) {
                    ...
                    if ($aRow['billable'] == 1 && $aRow['billed'] == 0) {
                ...
                if ($aColumns[$i] == 'start_time' || $aColumns[$i] == 'end_time') {
            ...
            if ($aRow['end_time'] !== NULL) {
                ...
                if ($aRow['status'] == 5) {
                    $icon_btn = '<span data-toggle="tooltip" data-title="' . _l('task_edit_delete_timesheet_notice', array(
                        ($task_is_billed ? _l('task_billed') : _l('task_status_5')),
                        _l('edit')
                    )) . '">' . $icon_btn . '</span>';
                }
                $options .= $icon_btn;
