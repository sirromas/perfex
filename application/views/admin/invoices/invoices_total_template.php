<div class="row">
   ...
   <div class="col-md-12 simple-bootstrap-select mbot5">
      ...
      <select data-show-subtext="true" data-width="auto"
			class="selectpicker" name="total_currency"
			onchange="init_invoices_total();">
         <?php
        
foreach ($invoices_total_currencies as $currency) {
            $selected = '';
...
if (count($invoices_years) > 1) {
        $currentYear = date('Y');
        ?>
      <select data-none-selected-text="<?php echo $currentYear; ?>"
			data-current-year="<?php echo $currentYear; ?>" data-width="auto"
			class="selectpicker" name="invoices_total_years"
			onchange="init_invoices_total();" multiple="true"
			id="invoices_total_years">
         <?php
        ...
        foreach ($invoices_years as $year) {
         ...
         <option value="<?php echo $year['year']; ?>"
				<?php if($selectedYears && in_array($year['year'], $selectedYears) || !$selectedYears && date('Y') == $year['year']){echo ' selected'; } ?>><?php echo $year['year']; ?></option>
         <?php } ?>
