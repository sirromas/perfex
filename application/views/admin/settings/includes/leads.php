...
<i class="fa fa-question-circle" data-toggle="tooltip"
	data-title="<?php echo _l('auto_assign_customer_admin_after_lead_convert_help'); ?>"></i>
...
<div class="row">
	<div class="col-md-7">
		...
		<select name="settings[default_leads_kanban_sort]"
			id="default_leads_kanban_sort" class="selectpicker" data-width="100%"
			data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
			<option value=""></option>
			<option value="dateadded"
				<?php if(get_option('default_leads_kanban_sort') == 'dateadded'){echo 'selected'; }?>><?php echo _l('leads_sort_by_datecreated'); ?></option>
			<option value="leadorder"
				<?php if(get_option('default_leads_kanban_sort') == 'leadorder'){echo 'selected'; }?>><?php echo _l('leads_sort_by_kanban_order'); ?></option>
			<option value="lastcontact"
				<?php if(get_option('default_leads_kanban_sort') == 'lastcontact'){echo 'selected'; }?>><?php echo _l('leads_sort_by_lastcontact'); ?></option>
		</select>
	</div>
	<div class="col-md-5">
		<div class="mtop30 text-right">
			<div class="radio radio-inline radio-primary">
				<input type="radio" id="k_desc"
					name="settings[default_leads_kanban_sort_type]" value="asc"
					<?php if(get_option('default_leads_kanban_sort_type') == 'asc'){echo 'checked';} ?>>
				<label for="k_desc"><?php echo _l('order_ascending'); ?></label>
			</div>
			<div class="radio radio-inline radio-primary">
				<input type="radio" id="k_asc"
					name="settings[default_leads_kanban_sort_type]" value="desc"
					<?php if(get_option('default_leads_kanban_sort_type') == 'desc'){echo 'checked';} ?>>
...
<div class="form-group">
  ...
  </label> <input type="text" id="settings[lead_modal_class]"
		name="settings[lead_modal_class]" class="form-control"
