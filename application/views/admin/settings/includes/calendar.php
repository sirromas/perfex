<ul class="nav nav-tabs" role="tablist">
	...
	<li role="presentation"><a href="#colors" aria-controls="colors"
		role="tab" data-toggle="tab"><?php echo _l('settings_calendar_colors_heading'); ?></a>
	</li>
</ul>
...
<div class="tab-content mtop30">
	<div role="tabpanel" class="tab-pane active" id="general">
		...
		<a href="<?php echo admin_url('departments'); ?>"
			class="mbot30 display-block"><?php echo _l('setup_calendar_by_departments'); ?></a>
    ...
    <hr />
		<div class="form-group">
			...
			<br /> <select class="selectpicker" data-width="100%"
				name="settings[default_view_calendar]" id="default_view_calendar">
				<option value="month"
					<?php if(get_option('default_view_calendar') == 'month'){echo ' selected';} ?>><?php echo _l('month'); ?></option>
				<option value="basicWeek"
					"<?php if(get_option('default_view_calendar') == 'basicWeek'){echo ' selected';} ?>><?php echo _l('week'); ?></option>
				<option value="basicDay"
					"<?php if(get_option('default_view_calendar') == 'basicDay'){echo ' selected';} ?>><?php echo _l('day'); ?></option>
				<option value="agendaWeek"
					"<?php if(get_option('default_view_calendar') == 'agendaWeek'){echo ' selected';} ?>><?php echo _l('agenda'); ?> <?php echo _l('week'); ?></option>
				<option value="agendaDay"
					"<?php if(get_option('default_view_calendar') == 'agendaDay'){echo ' selected';} ?>><?php echo _l('agenda'); ?> <?php echo _l('day'); ?></option>
			</select>
		</div>
		<hr />
		<label><?php echo _l('calendar_first_day'); ?></label> <select
			name="settings[calendar_first_day]" class="selectpicker"
			data-width="100%">
      <?php
    ...
    foreach ($weekdays as $key => $val) {
        ...
        <option value="<?php echo $key; ?>"
				<?php if(get_option('calendar_first_day') == $key){echo 'selected';} ?>><?php echo $val; ?></option>
        <?php } ?>
      </select>
		<hr />
