...
$settings_groups = array(
    ...
    foreach ($settings_groups as $group) {
        ...
        <li <?php if($i == 0){echo " class='active'"; } ?>><a
								href="<?php echo (!isset($group['url']) ? admin_url('settings?group='.$group['name']) : $group['url']) ?>"
								data-group="<?php echo $group['name']; ?>">
          <?php echo $group['lang']; ?></a></li>
        <?php $i++; } ?>
      </ul>
    ...
    <?php if(get_option('desktop_notifications') == '1'){ ?>
      ...
      if (!("Notification" in window)) {
        ...
        if(Notification.permission == "denied"){
          $('#pusherHelper').html('<div class="alert alert-danger">Desktop notifications not allowed in browser settings, search Google "How to allow desktop notifications <?php echo $this->agent->browser(); ?>"</div>');
        }
      }
      <?php } ?>
