...
class Pusher
{
    ...
    public function __construct($auth_key, $secret, $app_id, $options = array(), $host = null, $port = null, $timeout = null)
    {
        ...
        if (! is_null($host)) {
            ...
            $this->log('Legacy $host parameter provided: ' . $this->settings['scheme'] . ' host: ' . $this->settings['host']);
        }
        if (! is_null($port)) {
            $options['port'] = $port;
        }
        ...
        if (isset($options['encrypted']) && $options['encrypted'] === true && ! isset($options['scheme']) && ! isset($options['port'])) {
     ...
     *            An array of channel names to validate
     *            
    ...
    private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array())
    {
        ...
        $signed_query = self::build_auth_query_string($this->settings['auth_key'], $this->settings['secret'], $request_method, $s_url, $query_params);
     ...
     *            An array of channel names to publish the event on.
     * @param string $event            
     * @param mixed $data
     *            Event data
     * @param string $socket_id
     ...
     * @param array $params
     *            Additional parameters for the query e.g. $params = array( 'info' => 'connection_count' )
     *            
     * @return object
     */
     ...
     *
     * @param
     *            string path Path excluding /apps/APP_ID
     * @param
