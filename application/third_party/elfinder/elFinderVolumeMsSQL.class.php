...
class elFinderVolumeMsSQL extends elFinderVolumeDriver
{
    ...
    {
        if (! ($this->options['host']) || ! $this->options['user'] || ! $this->options['pass'] || ! $this->options['db'] || ! $this->options['path'] || ! $this->options['files_table']) {
            return false;
        }
     ...
     *            file path
     * @param string $mode
     *            open file mode (ignored in this driver)
    ...
    {
        $fp = $this->tmbPath ? @fopen($this->getTempFile($path), 'w+') : @tmpfile();
    ...
    /**
     * Create symlink.
     * FTP driver does not support symlinks.
     *
     * @param string $target
     *            link target
     * @param string $path
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        $sql = $id > 0 ? sprintf('REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime, width, height, [read], write, locked, hidden) (SELECT %d, %d, name, content, size, mtime, mime, width, height, [read, write, locked, hidden FROM %s WHERE id=%d)', $this->tbf, $id, $this->_dirname($id), $this->tbf, $source) : sprintf('INSERT INTO %s (parent_id, name, content, size, mtime, mime, width, height, [read], write, locked, hidden) SELECT %d, "%s", content, size, %d, mime, width, height, [read], write, locked, hidden FROM %s WHERE id=%d', $this->tbf, $targetDir, addslashes($name), time(), $this->tbf, $source);
    ...
    protected function _save($fp, $dir, $name, $stat)
    {
        ...
        if (($tmpfile = tempnam($this->tmpPath, $this->id))) {
            if (($trgfp = fopen($tmpfile, 'wb')) == false) {
                ...
                $sql = $id > 0 ? 'REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime,[read], write, locked, hidden, width, height) VALUES (' . $id . ', %d, "%s", convert(varbinary(max),"%s")), %d, %d, "%s", 1, 1, 0, 0, %d, %d)' : 'INSERT INTO %s (parent_id, name, content, size, mtime, mime,[read], write, locked, hidden, width, height) VALUES (%d, "%s", convert(varbinary(max),"%s")), %d, %d, "%s", 1, 1, 0, 0, %d, %d)';
        ...
        $sql = $id > 0 ? 'REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime, [read], write, locked, hidden, width, height) VALUES (' . $id . ', %d, "%s", convert(varbinary(max),"%s"), %d, %d, "%s", 1, 1, 0, 1, %d, %d)' : 'INSERT INTO %s (parent_id, name, content, size, mtime, mime, [read], write, locked, hidden, width, height) VALUES (%d, "%s", convert(varbinary(max),"%s"), %d, %d, "%s", 1, 1, 0, 0, %d, %d)';
