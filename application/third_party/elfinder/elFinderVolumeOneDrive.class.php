...
class elFinderVolumeOneDrive extends elFinderVolumeDriver
{
    ...
    protected function _od_refreshToken()
    {
        if ($this->token->expires < time()) {
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS => 'client_id=' . urlencode($this->options['client_id']) . '&client_secret=' . urlencode($this->options['client_secret']) . '&grant_type=refresh_token' . '&refresh_token=' . urlencode($token->data->refresh_token),
        ...
        try {
            ...
            if (isset($_GET['code'])) {
                try {
                    ...
                    $this->session->set('OneDriveTokens', $this->_od_obtainAccessToken($options['client_id'], $options['client_secret'], $_GET['code']));
                    ...
                    $out = array(
                        ...
                        'json' => json_encode(array(
                            'error' => elFinder::ERROR_ACCESS_DENIED . ' ' . $e->getMessage()
                        ))
                    );
                ...
                $out = array(
                    'node' => $options['id'],
                    'json' => json_encode(array(
                        'error' => elFinder::ERROR_ACCESS_DENIED
                    ))
                );
            ...
            if ($options['user'] === 'init') {
                ...
                if (! $result) {
                    ...
                    $callback = $options['url'] . '?cmd=netmount&protocol=onedrive&host=onedrive.com&user=init&pass=return&node=' . $options['id'] . $cdata;
                    ...
                    try {
                        ...
                        $url = self::AUTH_URL . '?client_id=' . urlencode($options['client_id']) . '&scope=' . urlencode('wl.skydrive_update' . $offline) . '&response_type=code' . '&redirect_uri=' . urlencode($redirect_uri);
    ...
    protected function init()
    {
        ...
        if (empty($options['netkey'])) {
            ...
            $this->netMountKey = md5(implode('-', array(
                'box',
                $this->options['path'],
                $_tokenKey
            )));
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        return $path ? $path : $this->setError(elFinder::ERROR_COPY, $this->_path($src));
    }
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        try {
            ...
            if ($parentId === 'root') {
                $properties['parentReference'] = (object) array(
                    'path' => '/drive/root:'
                );
            } else {
                $properties['parentReference'] = (object) array(
