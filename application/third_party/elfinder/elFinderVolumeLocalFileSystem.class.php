...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        ...
        $this->options['keepTimestamp'] = array(
            'copy',
            'move'
        ); // keep timestamp at inner filesystem allowed 'copy', 'move' and 'upload'
    }
    ...
    protected function init()
    {
        ...
        if (is_null($this->options['syncCheckFunc'])) {
            $this->options['syncCheckFunc'] = array(
                $this,
                'localFileSystemInotify'
            );
        }
    ...
    protected function _subdirs($path)
    {
        ...
        if (is_dir($path)) {
            if (class_exists('FilesystemIterator', false)) {
                $dirItr = new ParentIterator(new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS | (defined('RecursiveDirectoryIterator::FOLLOW_SYMLINKS') ? RecursiveDirectoryIterator::FOLLOW_SYMLINKS : 0)));
                $dirItr->rewind();
    ...
    {
        clearstatcache();
        return strpos($mime, 'image') === 0 && is_readable($path) && ($s = getimagesize($path)) !== false ? $s[0] . 'x' . $s[1] : false;
    }
    ...
    protected function _extract($path, $arc)
    {
        if ($this->quarantine) {
            ...
            if ($symlinks) {
                $this->delTree($dir);
                return $this->setError(array_merge($this->error, array(
                    elFinder::ERROR_ARC_SYMLINKS
                )));
            }
