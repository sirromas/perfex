...
class elFinder
{
     ...
     *
     * @param
     *            array elFinder and roots configurations
     * @author Dmitry (dio) Levashov
     */
     ...
     *            string command name
     * @param
     *            string|array callback name or array(object, method)
     * @return elFinder
     * @author Dmitry (dio) Levashov
     *        
     */
    ...
    public function exec($cmd, $args)
    {
        ...
        if ($this->session_expires()) {
            return array(
                'error' => $this->error(self::ERROR_SESSION_EXPIRES)
            );
        }
        ...
        if (! empty($this->listeners[$cmd . '.pre'])) {
            foreach ($this->listeners[$cmd . '.pre'] as $handler) {
                $_res = call_user_func_array($handler, array(
                    $cmd,
                    &$args,
                    $this,
                    $dstVolume
        ...
        if (! empty($this->listeners[$cmd])) {
            foreach ($this->listeners[$cmd] as $handler) {
                if (call_user_func_array($handler, array(
                    $cmd,
                    &$result,
                    $args,
                    $this,
    ...
    protected function netmount($args)
    {
        ...
        if ($protocol === 'netunmount') {
            ...
            return array(
                'sync' => true,
                'error' => $this->error(self::ERROR_NETUNMOUNT)
            );
        }
        ...
        if (! class_exists($class)) {
            return array(
                'error' => $this->error(self::ERROR_NETMOUNT, $args['host'], self::ERROR_NETMOUNT_NO_DRIVER)
            );
        }
        ...
        if (! isset($options['id'])) {
            ...
            if (! $options['id'] = $this->getNetVolumeUniqueId($netVolumes)) {
                return array(
                    'error' => $this->error(self::ERROR_NETMOUNT, $args['host'], 'Could\'t given volume id.')
                );
            }
        }
        ...
        if ($volume->mount($options)) {
            ...
            return array(
                'error' => $this->error(self::ERROR_NETMOUNT, $args['host'], implode(' ', $volume->error()))
            );
        }
    }
    ...
    protected function mkdir($args)
    {
        ...
        if ($name === '' && ! $dirs) {
            return array(
                'error' => $this->error(self::ERROR_INV_PARAMS, 'mkdir')
            );
        }
        ...
        if ($dirs) {
            ...
            return ($res = $this->ensureDirsRecursively($volume, $target, $mkdirs)) === false ? array(
    ...
    protected function rename($args)
    {
        ...
        if ($this->itemLocked($target)) {
            return array(
                'error' => $this->error(self::ERROR_LOCKED, $rm['name'])
            );
        }
    ...
    protected function chmod($args)
    {
        ...
        foreach ($targets as $target) {
            ...
            if ($file) {
                $files = array_merge($files, is_array($file) ? $file : array(
                    $file
                ));
            } else {
    ...
    protected function upload($args)
    {
        ...
        $header = empty($args['html']) ? array() : array(
        ...
        if (strpos($chunk, '/') !== false || strpos($chunk, '\\') !== false) {
            return array(
                'error' => $this->error(self::ERROR_UPLOAD)
            );
        }
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            if ($path !== '' && $path !== $target) {
                ...
                if (isset($renames[$name])) {
                    ...
                    $rnres = $this->rename(array(
                        'target' => $hash,
                        'name' => $volume->uniqueName($dir, $name, $suffix, true, 0)
                    ));
        ...
        foreach ($targets as $target) {
            ...
            if ($renames) {
                ...
                if (isset($renames[$file['name']])) {
                    ...
                    $rnres = $this->rename(array(
                        'target' => $hash,
                        'name' => $dstVolume->uniqueName($dir, $file['name'], $suffix, true, 0)
                    ));
                    if (! empty($rnres['error'])) {
                        $result['warning'] = $rnres['error'];
                        break;
    ...
    protected function get($args)
    {
        ...
        if ($enc !== false) {
            ...
            if ($content === false || $json === false || strlen($json) < strlen($content)) {
                return array(
                    'error' => $this->error(self::ERROR_CONV_UTF8, self::ERROR_NOT_UTF8_CONTENT, $volume->path($target))
                );
            }
        }
    ...
    protected function extract($args)
    {
        ...
        return ($file = $volume->extract($target, $makedir)) ? array(
            'added' => isset($file['read']) ? array(
                $file
            ) : $file
        ) : array(
    ...
    protected function archive($args)
    {
        ...
        return ($file = $volume->archive($targets, $args['type'], $name)) ? array(
            'added' => array(
                $file
            )
        ) : array(
        ...
        if ($args['compare'] && count($args['targets']) === 1) {
            ...
            if ($volume = $this->volume($hash)) {
                ...
                if (($syncCheckFunc = $volume->getOption('syncCheckFunc')) && is_callable($syncCheckFunc)) {
                    $_compare = call_user_func_array($syncCheckFunc, array(
    ...
    protected function resize($args)
    {
        ...
        if ($mode !== 'rotate' && ($width < 1 || $height < 1)) {
            return array(
                'error' => $this->error(self::ERROR_RESIZESIZE)
            );
        }
        return ($file = $volume->resize($target, $width, $height, $x, $y, $mode, $bg, $degree, $quality)) ? array(
            'changed' => array(
                $file
            )
        ) : array(
     ...
     * @return array|false => array([stat of maked directory]), 'hashes' => array('[path]' => '[hash]'))
     * @author Naoki Sawada
     *        
     */
    ...
    public static function getConnectorUrl()
    {
        ...
        $url = ($https ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . // host
...
$_SERVER['REQUEST_URI']; // path & query
    ...
    public static function curlExec($curl, $options = array(), $headers = array())
    {
        ...
        if (! $followLocation && $redirect = curl_getinfo($curl, CURLINFO_REDIRECT_URL)) {
            if ($stream = self::getStreamByUrl(array(
                'target' => $redirect,
                'headers' => $headers
            ))) {
