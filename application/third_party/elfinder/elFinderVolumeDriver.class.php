...
abstract class elFinderVolumeDriver
{
     ...
     * @var array
     */
    protected $sessionCaching = array(
        'rootstat' => true,
        'subdirs' => true
    );
    ...
    protected function configure()
    {
        ...
        $type = preg_match('/^(imagick|gd|convert|auto)$/i', $this->options['imgLib']) ? strtolower($this->options['imgLib']) : 'auto';
    ...
    public function mount(array $opts)
    {
        ...
        if (is_string($this->options['uploadOrder'])) { // telephat_mode on, compatibility with 1.x
            ...
            $this->uploadOrder = ! empty($this->options['uploadOrder']) ? $this->options['uploadOrder'] : array(
                'deny',
                'allow'
            );
        }
        ...
        $this->disabled = isset($this->options['disabled']) && is_array($this->options['disabled']) ? array_values(array_diff($this->options['disabled'], array(
            'open'
        ))) : // 'open' is required
array();
     ...
     * @param string $name
     *            target option name
     * @return NULL|mixed option value
     * @author Naoki Sawada
     */
    ...
    public function paste($volume, $src, $dst, $rmSrc = false, $hashes = array())
    {
        ...
        if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
            return $rmSrc ? $this->setError($err, $errpath, elFinder::ERROR_LOCKED, $volume->path($test)) : $this->setError($err, $errpath, ! empty($file['thash']) ? elFinder::ERROR_PERM_DENIED : elFinder::ERROR_MKOUTLINK);
        }
    ...
    public function archive($hashes, $mime, $name = '')
    {
        ...
        if ($name === '') {
            ...
            $name = str_replace(array(
                '/',
                '\\'
            ), '_', preg_replace('/\.' . preg_quote($archiver['ext'], '/') . '$/i', '', $name));
        }
        $name .= '.' . $archiver['ext'];
    ...
    public function search($q, $mimes, $hash = null)
    {
        ...
        return ($q === '' || $this->commandDisabled('search')) ? array() : $this->doSearch(is_null($dir) ? $this->root : $dir, $q, $mimes);
    }
     ...
     * Tempfile will be removed when after script execution finishes or exit() is called.
     ...
     * @param string $canLink
     *            it can use link() (for recurcive)
     * @return string|false path name
     * @author Naoki Sawada
     */
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if ($root) {
            unset($stat['hidden']);
        } elseif ($this->attr($path, 'hidden', isset($stat['hidden']) ? ! ! $stat['hidden'] : null, $isDir) || ! $this->mimeAccepted($stat['mime'])) {
            $stat['hidden'] = 1;
        } else {
            unset($stat['hidden']);
        }
            ...
            if ($isDir) {
                ...
                if ($this->options['checkSubfolders']) {
                    if (isset($stat['dirs'])) {
                        ...
                        $stat['dirs'] = isset($this->cache[$stat['target']]) ? intval(isset($this->cache[$stat['target']]['dirs'])) : $this->subdirsCE($stat['target']);
    ...
    protected function countSize($path)
    {
        ...
        $result = 0;
        foreach ($this->getScandir($path) as $stat) {
            $size = $stat['mime'] == 'directory' && $stat['read'] ? $this->countSize($this->decode($stat['hash'])) : (isset($stat['size']) ? intval($stat['size']) : 0);
            if ($size > 0) {
                $result += $size;
            }
        }
    ...
    protected function isMyReload($target = '', $ARGtarget = '')
    {
        ...
        if (! empty($this->ARGS['reload'])) {
            if ($ARGtarget === '') {
                $ARGtarget = isset($this->ARGS['target']) ? $this->ARGS['target'] : ((isset($this->ARGS['targets']) && is_array($this->ARGS['targets']) && count($this->ARGS['targets']) === 1) ? $this->ARGS['targets'][0] : '');
            }
            if ($ARGtarget !== '') {
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        if (! empty($srcStat['thash'])) {
            ...
            $this->clearcache();
            return $stat && $this->symlinkCE($target, $dst, $name) ? $this->joinPathCE($dst, $name) : $this->setError(elFinder::ERROR_COPY, $this->path($srcStat['hash']));
        }
        ...
        if ((! $checkTmbPath || $this->tmbPathWritable) && (! $this->tmbPath || strpos($path, $this->tmbPath) === false)) // do not create thumnbnail for thumnbnail
{
            ...
            return $this->imgLib && ($type === 'image') && ($this->imgLib == 'gd' ? in_array($stat['mime'], array(
                'image/jpeg',
                'image/png',
                'image/gif',
                'image/x-ms-bmp'
    ...
    protected function createTmb($path, $stat)
    {
        ...
        // call imgConverter
        if ($imgConverter) {
            if (! call_user_func_array($imgConverter, array(
                $tmb,
                $stat,
                $this
            ))) {
     ...
     * @param int $height
     *            square height
     * @param int|string $align
     *            reserved
     * @param int|string $valign
     *            reserved
    ...
    protected function imgRotate($path, $degree, $bgcolor = '#ffffff', $destformat = null, $jpgQuality = null)
    {
        ...
        // try lossless rotate
        if ($degree % 90 === 0 && in_array($s[2], array(
            IMAGETYPE_JPEG,
            IMAGETYPE_JPEG2000
        ))) {
            $count = ($degree / 90) % 4;
    ...
    protected function getArchivers($use_cache = true)
    {
        ...
        if ($this->procExec('') === 0) {
            ...
            if ($ctar == 0) {
                ...
                if ($c == 0) {
                    $arcs['create']['application/x-xz'] = array(
                        'cmd' => 'tar',
                        'argc' => '-cJf',
                        'ext' => 'xz'
                    );
                    $arcs['extract']['application/x-xz'] = array(
                        'cmd' => 'tar',
                        'argc' => '-xJf',
                        'ext' => 'xz'
                    );
    ...
    protected function makeArchive($dir, $files, $name, $arc)
    {
        if ($arc['cmd'] === 'phpfunction') {
            if (is_callable($arc['argc'])) {
                call_user_func_array($arc['argc'], array(
                    $dir,
                    $files,
                    $name
                ));
