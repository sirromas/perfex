...
class elFinderVolumeDropbox extends elFinderVolumeDriver
{
    ...
    public function netmountPrepare($options)
    {
        ...
        if ($options['user'] === 'init') {
            ...
            if ($options['pass'] === 'init') {
                ...
                $this->session->set('DropboxTokens', array(
                    $_GET['uid'],
                    $tokens['token'],
                    $tokens['token_secret']
                ));
    ...
    protected function init()
    {
        ...
        // make net mount key
        $this->netMountKey = md5(join('-', array(
            'dropbox',
            $this->options['path']
        )));
    ...
    {
        return $this->query('update ' . $this->DB_TableName . ' set dat=' . $this->DB->quote(serialize($dat)) . ', isdir=' . ($dat['is_dir'] ? 1 : 0) . ' where path=' . $this->DB->quote(strtolower($this->_dirname($path))) . ' and fname=' . $this->DB->quote(strtolower($this->_basename($path))));
    }
        ...
        try {
            ...
            do {
                ...
                foreach ($_info['entries'] as $entry) {
                    ...
                    if (empty($entry[1])) {
                        $ptimes[$pkey] = isset($ptimes[$pkey]) ? max(array(
                            $now,
                            $ptimes[$pkey]
                        )) : $now;
            ...
            $this->DB->exec('update ' . $this->DB_TableName . ' set dat=' . $this->DB->quote(serialize(array(
                'cursor' => $cursor,
