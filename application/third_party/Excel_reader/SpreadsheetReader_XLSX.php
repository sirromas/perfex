...
class SpreadsheetReader_XLSX implements Iterator, Countable
{
        ...
        if (! $Format) {
            ...
            if (substr($Format['Code'], - 1) == '%') {
                ...
                if (preg_match('{(0|#)(,+)}', $Format['Code'], $Matches)) {
                    ...
                    // Removing the commas
                    $Format['Code'] = preg_replace(array(
                        '{0,+}',
                        '{#,+}'
                    ), array(
                        '0',
     ...
     * of rows read so far.
     * It's not really correct but at least coherent.
     */
