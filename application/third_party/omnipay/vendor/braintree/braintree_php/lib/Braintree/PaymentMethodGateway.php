...
class PaymentMethodGateway
{
    ...
    public function grant($sharedPaymentMethodToken, $attribs = [])
    {
        ...
        return $this->_doCreate('/payment_methods/grant', [
    ...
    {
        return $this->_doCreate('/payment_methods/revoke', [
            'payment_method' => [
                'shared_payment_method_token' => $sharedPaymentMethodToken
            ]
        ]);
                                ...
                                if (isset($response['europeBankAccount'])) {
                                    ...
                                    if (isset($response['usBankAccount'])) {
                                        ...
                                        if (isset($response['venmoAccount'])) {
                                            ...
                                            if (isset($response['visaCheckoutCard'])) {
                                                return new Result\Successful(VisaCheckoutCard::factory($response['visaCheckoutCard']), "paymentMethod");
                                            } else 
                                                if (isset($response['masterpassCard'])) {
                                                    return new Result\Successful(MasterpassCard::factory($response['masterpassCard']), "paymentMethod");
                                                } else 
    ...
    {
        if (empty($identifier)) {
            throw new InvalidArgumentException('expected payment method id to be set');
        }
        if (! preg_match('/^[0-9A-Za-z_-]+$/', $identifier)) {
            throw new InvalidArgumentException($identifier . ' is an invalid payment method ' . $identifierType . '.');
        }
    }
}
