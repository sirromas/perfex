...
class CIMGatewayTest extends GatewayTestCase
{
    ...
    {
        $this->assertEquals('https://api2.authorize.net/xml/v1/request.api', $this->gateway->getLiveEndpoint());
    }

    public function testDeveloperEndpoint()
    {
        $this->assertEquals('https://apitest.authorize.net/xml/v1/request.api', $this->gateway->getDeveloperEndpoint());
    }

    public function testCreateCardSuccess()
    {
        $this->setMockHttpResponse(array(
            'CIMCreateCardSuccess.txt',
            'CIMGetPaymentProfileSuccess.txt'
        ));
        
        $response = $this->gateway->createCard($this->createCardOptions)->send();
        
        $this->assertTrue($response->isSuccessful());
        $this->assertSame('{"customerProfileId":"28972084","customerPaymentProfileId":"26485433"}', $response->getCardReference());
    ...
    {
        $this->setMockHttpResponse(array(
            'CIMCreateCardFailureWithDuplicate.txt',
            'CIMCreatePaymentProfileSuccess.txt',
            'CIMGetProfileSuccess.txt',
            'CIMGetPaymentProfileSuccess.txt'
        ));
        ...
        $this->assertTrue($response->isSuccessful());
        $this->assertSame('{"customerProfileId":"28775801","customerPaymentProfileId":"26485433"}', $response->getCardReference());
        $this->assertSame('Successful.', $response->getMessage());
    }
    ...
    public function testShouldUpdateExistingPaymentProfileIfDuplicateExistsAndForceCardUpdateIsSet()
    {
        ...
        $this->setMockHttpResponse(array(
            'CIMCreateCardFailureWithDuplicate.txt',
            'CIMCreatePaymentProfileFailure.txt',
            'CIMGetProfileSuccess.txt',
            'CIMUpdatePaymentProfileSuccess.txt',
            'CIMGetPaymentProfileSuccess.txt'
        ));
    ...
    public function testShouldUpdateExistingPaymentProfileIfDuplicateExistsAndMaxPaymentProfileLimitIsMet()
    {
        $this->setMockHttpResponse(array(
            'CIMCreateCardFailureWithDuplicate.txt',
            'CIMCreatePaymentProfileFailureMaxProfileLimit.txt',
            'CIMGetProfileSuccess.txt',
            'CIMUpdatePaymentProfileSuccess.txt',
            'CIMGetPaymentProfileSuccess.txt'
        ));
