...
class AIMAbstractRequestTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->request = $this->getMockForAbstractClass('\Omnipay\AuthorizeNet\Message\AIMAbstractRequest', array(
    ...
    {
        $complexKey = json_encode(array(
            'transId' => 'TRANS_ID',
            'cardReference' => 'CARD_REF'
        ));
        $this->request->setTransactionReference($complexKey);
        $this->assertEquals('TRANS_ID', $this->request->getTransactionReference()
            ->getTransId());
    }
