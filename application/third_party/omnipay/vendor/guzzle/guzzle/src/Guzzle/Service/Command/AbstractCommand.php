...
abstract class AbstractCommand extends Collection implements CommandInterface
{
    ...
    public function prepare()
    {
        if (! $this->isPrepared()) {
            ...
            $this->client->dispatch('command.before_prepare', array(
                'command' => $this
            ));
            ...
            $this->client->dispatch('command.after_prepare', array(
                'command' => $this
            ));
        }
    ...
    /**
     * Set the validator used to validate and prepare command parameters and nested JSON schemas.
     * If no validator is
    ...
    /**
     * Get the validator used to prepare and validate properties.
     * If no validator has been set on the command, then
