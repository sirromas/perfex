 ...
 * Caching can be done on all requests passing through this
...
class CachePlugin implements EventSubscriberInterface
{
    ...
    /**
     *
     * @param array|CacheAdapterInterface|CacheStorageInterface $options
    ...
    public function __construct($options = null)
    {
        if (! is_array($options)) {
            if ($options instanceof CacheAdapterInterface) {
                $options = array(
                    'storage' => new DefaultCacheStorage($options)
                );
                ...
                $options = array(
                    'storage' => new DefaultCacheStorage(CacheAdapterFactory::fromCache($options))
                );
