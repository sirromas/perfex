 ...
 * Can redirect like a web browser or using strict RFC 2616 compliance
 */
class RedirectPlugin implements EventSubscriberInterface
{
    ...
    public function onRequestSent(Event $event)
    {
        ...
        if (! $response->isRedirect() || ! $response->hasHeader('Location')) {
            if ($request !== $original) {
                ...
                $response->getParams()->set(self::REDIRECT_COUNT, $original->getParams()
                    ->get(self::REDIRECT_COUNT));
     ...
     * @param RequestInterface $original
     *            Original request
    ...
    protected function createRedirectRequest(RequestInterface $request, $statusCode, $location, RequestInterface $original)
    {
        ...
        {
            $redirectRequest->getEventDispatcher()
                ->removeListener('request.before_send', $func);
            $e['request']->getParams()
                ->set(RedirectPlugin::PARENT_REQUEST, $request);
        });
    ...
    protected function throwTooManyRedirectsException(RequestInterface $original, $max)
    {
        $original->getEventDispatcher()->addListener('request.complete', $func = function ($e) use(&$func, $original, $max)
        {
            $original->getEventDispatcher()
                ->removeListener('request.complete', $func);
