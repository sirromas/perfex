...
class Server
{
    ...
    public function isRunning()
    {
        ...
        try {
            $this->client->get('guzzle-server/perf', array(), array(
                'timeout' => 5
            ))->send();
            $this->running = true;
            return true;
     ...
     *
     * @param bool $hydrate
     *            Set to TRUE to turn the messages into
    ...
    public function start()
    {
        if (! $this->isRunning()) {
            ...
            if (! $this->running) {
                throw new RuntimeException('Unable to contact server.js. Have you installed node.js v0.5.0+? node must be in your path.');
            }
        }
    }
