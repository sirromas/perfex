...
class RedirectPluginTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testRedirectsRequests()
    {
        ...
        $this->assertEquals(2, $request->getParams()
            ->get(RedirectPlugin::REDIRECT_COUNT));
    ...
    public function testCanLimitNumberOfRedirects()
    {
        ...
        try {
            ...
            $this->assertContains("5 redirects were issued for this request:\nGET /foo HTTP/1.1\r\n", $e->getMessage());
        }
    }
    ...
    public function testCanRedirectWithNoLeadingSlashAndQuery()
    {
        ...
        $this->assertEquals($this->getServer()
            ->getUrl() . '?foo=bar', $requests[0]->getUrl());
        $this->assertEquals($this->getServer()
            ->getUrl() . 'redirect?foo=bar', $requests[1]->getUrl());
        // Ensure that the history on the actual request is correct
        $this->assertEquals($this->getServer()
            ->getUrl() . '?foo=bar', $request->getUrl());
    }
    ...
    public function testResetsHistoryEachSend()
    {
        ...
        $this->assertTrue($request->getParams()
            ->hasKey('redirect.count'));
        $this->assertContains('/redirect2', $response->getEffectiveUrl());
        
        $request->send();
        $this->assertFalse($request->getParams()
            ->hasKey('redirect.count'));
    }
