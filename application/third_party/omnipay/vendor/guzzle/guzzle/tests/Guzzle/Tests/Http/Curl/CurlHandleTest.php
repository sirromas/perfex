...
class CurlHandleTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testStoresStdErr()
    {
        ...
        $this->assertEquals($h->getStderr(true), $h->getOptions()
            ->get(CURLOPT_STDERR));
    ...
    {
        $h = new CurlHandle(curl_init('http://test.com'), array(
    ...
    public function testWrapsErrorsAndInfo()
    {
        ...
        $this->assertEquals($this->getServer()
            ->getUrl(), $h->getInfo(CURLINFO_EFFECTIVE_URL));
    ...
    public function testWrapsCurlOptions()
    {
        ...
        $this->assertEquals(true, $h->getOptions()
            ->get(CURLOPT_AUTOREFERER));
        $this->assertEquals(1024, $h->getOptions()
            ->get(CURLOPT_BUFFERSIZE));
    }
    ...
    public function dataProvider()
    {
        ...
        $tests = array(
            ...
            array(
                ...
                null,
                EntityBody::factory(fopen($testFile, 'r+')),
                array(
                    CURLOPT_RETURNTRANSFER => 0,
                    CURLOPT_HEADER => 0,
                    CURLOPT_CONNECTTIMEOUT => 150,
    ...
    public function testUploadsPutDataUsingChunkedEncodingWhenLengthCannotBeDetermined()
    {
        ...
        $client = new Client($this->getServer()->getUrl());
        $request = $client->put('/');
        $request->setBody(EntityBody::factory(fopen($this->getServer()
            ->getUrl(), 'r')), 'text/plain');
        $request->send();
    ...
    public function testSendsPostRequestsWithFields()
    {
        ...
        $request = RequestFactory::getInstance()->create('POST', $this->getServer()
            ->getUrl());
        $request->getCurlOptions()->set('debug', true);
    ...
    public function testCurlConfigurationOptionsAreSet()
    {
        ...
        $handle = CurlHandle::factory($request);
        $this->assertEquals(99, $handle->getOptions()
            ->get(CURLOPT_CONNECTTIMEOUT));
        $this->assertEquals(8181, $handle->getOptions()
            ->get(CURLOPT_PORT));
        $this->assertNull($handle->getOptions()
            ->get('curl.fake_opt'));
        $this->assertNull($handle->getOptions()
            ->get('fake_opt'));
    }
    ...
    public function testAllowsWireTransferInfoToBeEnabled()
    {
        ...
        $request->getCurlOptions()->set('debug', true);
        $handle = CurlHandle::factory($request);
        $this->assertNotNull($handle->getOptions()
            ->get(CURLOPT_STDERR));
        $this->assertNotNull($handle->getOptions()
            ->get(CURLOPT_VERBOSE));
    }
    ...
    public function testAddsCustomCurlOptions()
    {
        ...
        $handle = CurlHandle::factory($request);
        $this->assertEquals(200, $handle->getOptions()
            ->get(CURLOPT_TIMEOUT));
    }
    ...
    public function testSendsPostFieldsForNonPostRequests()
    {
        ...
        $client = new Client();
        $request = $client->put($this->getServer()
            ->getUrl(), null, array(
            'foo' => 'baz',
            'baz' => 'bar'
        ));
        ...
        $this->assertEquals('application/x-www-form-urlencoded; charset=utf-8', (string) $requests[0]->getHeader('Content-Type'));
