...
class ClientTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    {
        return new LogPlugin(new ClosureLogAdapter(function ($message, $priority, $extras = null)
        {
    ...
    public function testClientAddsParamsToRequests()
    {
        ...
        $this->assertEquals('bar', $request->getParams()
            ->get('foo'));
        $this->assertEquals('jar', $request->getParams()
            ->get('baz'));
    ...
    public function urlProvider()
    {
        ...
        return array(
            ...
            array(
                ...
                $this->getServer()->getUrl() . 'absolute/path/to/resource'
            ),
            array(
                $u,
    ...
    public function testBuildsRelativeUrls($baseUrl, $url, $result)
    {
        ...
        $this->assertEquals($result, $client->get($url)
            ->getUrl());
    }
    ...
    public function testCreatesRequestsWithDefaultValues()
    {
        ...
        $this->assertEquals($request->getUrl(), $this->getServer()
            ->getUrl() . 'path/1?q=2');
    }
    ...
    public function testClientHasHelperMethodsForCreatingRequests()
    {
        ...
        $this->assertEquals('PATCH', $client->patch()
            ->getMethod());
        $this->assertEquals($url . 'base/abc', $client->get('abc')
            ->getUrl());
        $this->assertEquals($url . 'zxy', $client->put('/zxy')
            ->getUrl());
        $this->assertEquals($url . 'zxy?a=b', $client->post('/zxy?a=b')
            ->getUrl());
        $this->assertEquals($url . 'base?a=b', $client->head('?a=b')
            ->getUrl());
        $this->assertEquals($url . 'base?a=b', $client->delete('/base?a=b')
            ->getUrl());
    }
    ...
    {
        $mock = $this->getMock('Guzzle\\Http\\Curl\\CurlMulti', array(
            'add',
            'send'
        ));
    ...
    public function testQueryStringsAreNotDoubleEncoded()
    {
        ...
        $this->assertEquals('hi there', $request->getQuery()
            ->get('query'));
    ...
    public function testAllowsCustomVariablesWhenExpandingTemplates()
    {
        ...
        $ref = new \ReflectionMethod($client, 'expandTemplate');
        $ref->setAccessible(true);
        $uri = $ref->invoke($client, 'http://{test}{?query*}', array(
            'query' => array(
                'han' => 'solo'
            )
        ));
    ...
    public function testUriArrayAllowsCustomTemplateVariables()
    {
        ...
        $this->assertEquals('/hi', (string) $client->createRequest('GET', array(
            '/{var}',
            $vars
        ))
            ->getUrl());
        $this->assertEquals('/hi', (string) $client->get(array(
            '/{var}',
            $vars
        ))
            ->getUrl());
        $this->assertEquals('/hi', (string) $client->put(array(
            '/{var}',
            $vars
        ))
            ->getUrl());
        $this->assertEquals('/hi', (string) $client->post(array(
            '/{var}',
            $vars
        ))
            ->getUrl());
        $this->assertEquals('/hi', (string) $client->head(array(
            '/{var}',
            $vars
        ))
            ->getUrl());
        $this->assertEquals('/hi', (string) $client->options(array(
            '/{var}',
            $vars
        ))
            ->getUrl());
    ...
    public function testAllowsDefaultHeaders()
    {
        ...
        $this->assertEquals($default, $client->getDefaultHeaders()
            ->getAll());
    ...
    {
        $client = new Client();
        $request = $client->createRequest('GET', 'http://www.foo.com', array(
            'User-agent' => 'foo'
        ));
    ...
    {
        $client = new Client();
        $head = $client->head('http://www.foo.com', array(), array(
            'query' => array(
                'foo' => 'bar'
            )
        ));
        $this->assertEquals('bar', $head->getQuery()
            ->get('foo'));
    }
    ...
    public function testCanSetRelativeUrlStartingWithHttp()
    {
        ...
        $this->assertEquals('http://www.foo.com/httpfoo', $client->createRequest('GET', 'httpfoo')
            ->getUrl());
    }
}
