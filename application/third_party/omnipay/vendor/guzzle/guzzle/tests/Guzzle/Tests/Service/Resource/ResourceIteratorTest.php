...
class ResourceIteratorTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testSendsRequestsForNextSetOfResources()
    {
        ...
        $ri = new MockCommandIterator($this->getServiceBuilder()
            ->get('mock')
            ->getCommand('iterable_command'), array(
            'page_size' => 3
        ));
        ...
        $this->assertEquals(3, $requests[0]->getQuery()
            ->get('page_size'));
        $this->assertEquals(3, $requests[1]->getQuery()
            ->get('page_size'));
        $this->assertEquals(3, $requests[2]->getQuery()
            ->get('page_size'));
    ...
    public function testCalculatesPageSize()
    {
        ...
        $this->assertEquals(3, $requests[1]->getQuery()
            ->get('page_size'));
        $this->assertEquals(1, $requests[2]->getQuery()
            ->get('page_size'));
    }
