...
class Clients extends Clients_controller
{
    ...
    public function get_calendar_data()
    {
        ...
        $data = $this->utilities_model->get_calendar_data($this->input->get('start'), $this->input->get('end'), get_user_id_by_contact_id(get_contact_user_id()), get_contact_user_id());
                    ...
                    if (! $task_id) {
                        ...
                        if ($task_id) {
                            foreach ($assignees as $assignee) {
                                $this->tasks_model->add_task_assignees(array(
                                    'taskid' => $task_id,
                                    'assignee' => $assignee
                                ), false, true);
                            }
                            ...
                            if ($uploadedFiles && is_array($uploadedFiles)) {
                                foreach ($uploadedFiles as $file) {
                                    ...
                                    $this->misc_model->add_attachment_to_database($task_id, 'task', array(
                                        $file
                                    ));
                                }
                            }
                        ...
                        if ($project->settings->edit_tasks == 1 && total_rows('tblstafftasks', array(
                            ...
                            if ($totalAssignees == 0 && $project->settings->view_team_members == 1) {
                                ...
                                foreach ($assignees as $assignee) {
                                    if (! $this->tasks_model->is_task_assignee($assignee, $task_id)) {
                                        if ($this->tasks_model->add_task_assignees(array(
                                            'taskid' => $task_id,
                                            'assignee' => $assignee
                                        ), false, true)) {
                                            $affectedRows ++;
    ...
    public function statement()
    {
        ...
        $data['period_selected'] = json_encode(array(
            $from,
            $to
        ));
