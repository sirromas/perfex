				...
				<div class="panel_s">
					<div class="panel-body">
						<h4 class="no-margin">
								...
								href="<?php echo site_url('viewinvoice/'. $data['invoice']->id . '/' . $data['invoice']->hash); ?>">
						...
						<h4 class="mbot20">
							...
							value="<?php echo $data['key'] ?>" /> <input type="hidden"
							name="hash" value="<?php echo $data['hash'] ?>" /> <input
							type="hidden" name="txnid" value="<?php echo $data['txnid'] ?>" />
						<input type="hidden" name="amount"
							value="<?php echo $data['total'] ?>" /> <input type="hidden"
							name="surl"
						...
						<input type="hidden" name="service_provider" value="payu_paisa"
							size="64" /> <input type="hidden" name="productinfo"
							value="<?php echo $this->payu_money_gateway->getSetting('description_dashboard') . ' - ' . format_invoice_number($data['invoice']->id); ?>" />
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="last_name"
								name="lastname" value="<?php echo $data['lastname']; ?>">
						</div>

						<div class="form-group">
							<label for="email"> <?php echo _l('client_email'); ?> </label> <input
						...
						<div class="form-group">
                            ...
                            <?php if(!$data['hash']){ ?>
                                 <input type="submit"
							class="btn btn-info" value="<?php echo _l('submit_payment'); ?>" />
                            <?php } ?>
                             </form>
					</div>
				</div>
    ...
    public function success()
    {
        ...
        if (! $hashInfo) {
            ...
            if ($hashInfo['status'] == 'success') {
                $success = $this->payu_money_gateway->addPayment(array(
