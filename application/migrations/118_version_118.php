...
class Migration_Version_118 extends CI_Migration
{
    ...
    public function up()
    {
            ...
            if ($leads_email) {
                if (! empty($leads_email->password)) {
                    ...
                    $this->db->update('tblleadsemailintegration', array(
                        'password' => $this->encryption->encrypt($leads_email->password)
                    ));
                }
            }
        }
        ...
        if (is_object($menu)) {
            ...
            foreach ($menu->aside_menu_active as $item) {
                if ($item->id == 'tickets') {
                    ...
                    if ($item->id == 'child-new-ticket' || $item->id == 'child-open' || $item->id == 'child-in-progress' || $item->id == 'child-answered' || $item->id == 'child-on-hold' || $item->id == '"child-closed' || $item->id == 'child-all-tickets') {
        ...
        if ($default_format == 'Y-m-d|yyyy-mm-dd') {
            ...
            if ($default_format == 'm/d/Y|mm/dd/yyyy') {
                $f = 'm/d/Y|%m/%d/%Y';
            } else 
                if ($default_format == 'Y/m/d|yyyy/mm/dd') {
                    $f = 'Y/m/d|%Y/%m/%d';
                } else 
                    if ($default_format == 'd.m.Y|dd.mm.yyyy') {
                        $f = 'd.m.Y|%d.%m.%Y';
                    } else 
                        if ($default_format == 'd/m/Y|dd/mm/yyyy') {
                            $f = 'd/m/Y|%d/%m/%Y';
